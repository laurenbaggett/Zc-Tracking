...
for wn = 1:length(whale)
        if isempty(whale{wn}) % if no whale with this num
            ...
            scatter(whale{wn}.wloc(:,2),whale{wn}.wloc(:,1),[],brushing.params.colorMat(wn+2, :),'filled')% brushing.params.colorMat(wn+2, :))
...
for wn = 1:length(whale)
    if isempty(whale{wn})
        ...
        scatter(time,whale{wn}.wloc(:,3)+h0(3),[],brushing.params.colorMat(wn+2, :),'filled') % ,'color',brushing.params.colorMat(wn+2, :),'linewidth',3)
